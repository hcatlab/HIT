...
{-| Send a shared message from a page or layout.
-}
sendShared : Shared.Msg.Msg -> Effect msg
sendShared =
    SendSharedMsg


{-| Save auth payload (token + user) into local storage.
-}
saveAuth : Api.Auth.AuthResponse -> Effect msg
saveAuth authResponse =
    SendToLocalStorage
        { key = "hit"
        , value =
            Json.Encode.object
                [ ( "token", Json.Encode.string authResponse.token )
                , ( "user"
                  , Json.Encode.object
                        [ ( "id", Json.Encode.string authResponse.user.id )
                        , ( "email", Json.Encode.string authResponse.user.email )
                        ]
                  )
                ]
        }

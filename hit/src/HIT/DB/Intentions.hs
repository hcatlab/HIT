...
createIntention :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Connection -> UUID -> Text -> Text -> Maybe Text -> Fraction -> Deadline p -> NonEmpty UUID -> IO (Intention p)
...
getIntention :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Connection -> Text -> UUID -> IO (Maybe (Intention p))
...
getIntentionWithGoals :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Connection -> Text -> UUID -> IO (Maybe (Intention p, [UUID]))
...
listIntentionsWithGoals :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Connection -> Text -> IO [(Intention p, [UUID])]
listIntentionsWithGoals conn uid = do
  is <- listIntentions @p conn uid
  mapM (\i -> (i,) <$> listIntentionGoals conn uid (Intention.id i)) is

updateIntention :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Connection -> Text -> UUID -> Text -> Maybe Text -> Fraction -> Deadline p -> NonEmpty UUID -> IO (Maybe (Intention p))
...
deleteIntention :: forall p. (IntentionTableSelector p, DeadlineCodec p, IntervalTag p) => Proxy p -> Connection -> Text -> UUID -> IO Bool
